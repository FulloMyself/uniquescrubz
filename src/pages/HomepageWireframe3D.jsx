...
import { OrbitControls, Html, useProgress as DreiUseProgress } from "@react-three/drei";
...
function MovingPerson({ startX = -5, endX = 5, z = 0, speed = 0.02, scale = 1.2 }) {
  ...
  useFrame(() => {
    ...
    if (cloned) {
      ...
      cloned.position.z = z;
      cloned.position.y = -1;
      // Flip the rotation so the person faces the direction they're moving
...
function Loader() {
  const { progress } = DreiUseProgress();
  return (
    <Html center>
      <div style={{
        color: "gold",
